...
const defaultHideFunc = (node, time) => {
  node.changeTo(
    {
      alpha: 0,
    },
...
const removeChild = (parent, index) => {
  ...
  const { hide } = shouldBeRemoved;
  if (hide !== undefined) {
    parent.delayed += 1;
    shouldBeRemoved.isRemoving = true;
    if (typeof hide === 'function') {
      shouldBeRemoved.hide(() => {
        shouldBeRemoved.parent.delayed -= 1;
        shouldBeRemoved._remove();
      });
    } else {
      defaultHideFunc(shouldBeRemoved, parseInt(hide, 10));
    }
