...
const Render = {
  ...
  formatClass(props, allClass) {
    ...
    if (typeof cls === 'string') {
      ...
      cls.split(' ').reverse().forEach(_class => {
  ...
  update(realNode, type, oldProps, patches, options, formatted) {
    ...
    const isReRender = Object.keys(list).some((id) => {
      ...
      return typeof list[id] === 'function' ? list[id](realNode, formattedPatches[id], oldData[id], options) : list[id];
    });
    if (!isReRender) {
  ...
  getNodeZone(node) {
    if (node.$width !== undefined) {
      return [0, 0, node.$width, node.$height];
    }
    return [0, 0, node.width, node.height];
  },
  /**
  ...
  fit(node, fit) {
    ...
    if (fit === 'parent') {
      if (node.parent) {
        node.parent.on('$update', () => {
          console.log('fit!');
          console.log(this.getNodeZone(node.parent));
          this.fit(node, 'parent');
        });
        fit = {
          zone: this.getNodeZone(node.parent),
        };
      }
      else {
        node.once('added', () => {
...
Render.register('container', {
  ...
  /**
   * Container的width和height是由children决定的，
   * 直接设置值会让画面扭曲，一般情况不用，在此用$width,$height表明其实际大小
   */
  formatProps: {
    width(value, options, newProps, newData) {
      newData.$width = value;
      return undefined;
    },
    height(value, options, newProps, newData) {
