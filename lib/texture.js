    ...
    $options.sprite.forEach(e => {
      loader.add(e.src, (resource) => {
        ...
        if (!(texture instanceof Texture)) {
          data = 'ERROR';
          console.error('成功加载资源，但资源不是图片，可能是vue调试中错误路径重定向到主页面', resource.data);
        }
        e.nodes.forEach(node => {
          Render.update(node, 'sprite', { texture: null }, { texture: data }, $options, true);
        });
      });
      loader.onError.add((err) => {
        e.nodes.forEach(node => {
          Render.update(node, 'sprite', { texture: null }, { texture: 'ERROR' }, $options, true);
        });
