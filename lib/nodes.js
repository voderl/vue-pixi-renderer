...
const nodes = {
  ...
  setNode(node, options = {}) {
    const { start, event, data, update, destroy, init, show } = options;
...
// 注册节点类型
nodes.registerRender(['Sprite', 'Text', 'Graphics', 'TilingSprite']);
...
nodes.register('sprite', {
  Render(Renders, { animate, texture, time }) {
